...
class OrdersService {
  ...
  Future<Resource<List<Order>>> getOrders() async {
    try {
      Uri url = Uri.http(ApiConfig.API_PROYECTO, '/orders');
  ...
  Future<Resource<List<Order>>> getOrdersByClient(int idClient) async {
    try {
      Uri url = Uri.http(ApiConfig.API_PROYECTO, '/orders/$idClient');
      Map<String, String> headers = {"Content-Type": "application/json", "Authorization": await token};
      final response = await http.get(url, headers: headers);
      final data = json.decode(response.body);
      if (response.statusCode == 200 || response.statusCode == 201) {
  ...
  Future<Resource<Order>> updateStatus(int id) async {
    try {
      print('Id order: $id');
      Uri url = Uri.http(ApiConfig.API_PROYECTO, '/orders/$id');
      Map<String, String> headers = {"Content-Type": "application/json", "Authorization": await token};
